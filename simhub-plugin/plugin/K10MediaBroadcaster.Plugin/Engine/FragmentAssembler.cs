...
namespace K10MediaBroadcaster.Plugin.Engine
{
    ...
    public class FragmentAssembler
    {
        ...
        public void LoadFragments(string jsonPath)
        {
            ...
            {
                SimHub.Logging.Current.Warn($"[K10MediaBroadcaster.FragmentAssembler] Fragments file not found: {jsonPath}");
                return;
            }
            ...
            try
            {
                ...
                SimHub.Logging.Current.Info($"[K10MediaBroadcaster.FragmentAssembler] Loaded {_fragments.Count} fragment topics from {jsonPath}");
            }
            catch (Exception ex)
            {
                SimHub.Logging.Current.Error($"[K10MediaBroadcaster.FragmentAssembler] Failed to load fragments: {ex.Message}");
            }
        }
