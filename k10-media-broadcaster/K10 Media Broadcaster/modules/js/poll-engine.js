...
// Depends on: config.js (SIMHUB_URL, POLL_MS, PROP_KEYS)
  ...
  // Assuming SIMHUB_URL, POLL_MS, PROP_KEYS are already loaded

  // (Removed duplicate const PROP_KEYS = [ ... ] — defined in config.js)
  ...
  // All state variables are defined in config.js
  // Game detection functions are in game-detect.js (detectGameId, getGameFeatures, etc.)
  // Manufacturer detection (detectMfr) is in config.js (_mfrMap)
  // Connection banner variables are initialized below as needed
  ...
  Promise.all([loadCarLogos(), loadCountryFlags()]).then(() => {
    ...
    // Load settings and Discord state
    loadSettings().catch(e => console.warn('[K10] Settings load error:', e));
    initDiscordState().catch(e => console.warn('[K10] Discord init error:', e));
